...
class EnergyEndpointer {
 public:
  ...
  void ProcessAudioFrame(qint64 time_us,
                         const qint16* samples, int num_samples,
                         float* rms_out);

  // Returns the current state of the EnergyEndpointer and the time
  // corresponding to the most recently computed frame.
  EpStatus Status(qint64* status_time_us) const;
 ...
 private:
  ...
  qint64 endpointer_time_us_;  // Time of the most recently received audio frame.
  qint64 fast_update_frames_; // Number of frames for initial level adaptation.
  qint64 frame_counter_;  // Number of frames seen. Used for initial adaptation.
  ...
  boost::scoped_ptr<HistoryRing> history_;
  ...
  qint64 user_input_start_time_us_;

  Q_DISABLE_COPY(EnergyEndpointer)
};
