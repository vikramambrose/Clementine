...
float RMS(const qint16* samples, int num_samples) {
  qint64 ssq_int64 = 0;
  qint64 sum_int64 = 0;
...
qint64 Secs2Usecs(float seconds) {
  return static_cast<qint64>(0.5 + (1.0e6 * seconds));
}
...
class EnergyEndpointer::HistoryRing {
 public:
  ...
  void Insert(qint64 time_us, bool decision);
...
void EnergyEndpointer::HistoryRing::Insert(qint64 time_us, bool decision) {
...
qint64 EnergyEndpointer::HistoryRing::EndTime() const {
...
float EnergyEndpointer::HistoryRing::RingSum(float duration_sec) {
  ...
  if (ind < 0)
    ind = decision_points_.size() - 1;
  qint64 end_us = decision_points_[ind].time_us;
  bool is_on = decision_points_[ind].decision;
  qint64 start_us = end_us - static_cast<qint64>(0.5 + (1.0e6 * duration_sec));
  if (start_us < 0)
    start_us = 0;
...
int EnergyEndpointer::TimeToFrame(float time) const {
  return static_cast<qint32>(0.5 + (time / params_.frame_period()));
}
...
void EnergyEndpointer::Init(const EnergyEndpointerParams& params) {
  ...
  noise_level_ = params_.decision_threshold() / 2.0f;
  fast_update_frames_ =
      static_cast<qint64>(params_.fast_update_dur() / params_.frame_period());
...
void EnergyEndpointer::ProcessAudioFrame(qint64 time_us,
...
EpStatus EnergyEndpointer::Status(qint64* status_time)  const {
