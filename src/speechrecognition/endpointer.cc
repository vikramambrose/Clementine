...
Endpointer::Endpointer(int sample_rate)
  ...
  speech_input_minimum_length_us_ =
      static_cast<qint64>(1.7 * kUsecPerSec);
  speech_input_complete_silence_length_us_ =
      static_cast<qint64>(0.5 * kUsecPerSec);
...
EpStatus Endpointer::Status(qint64 *time) {
  return energy_endpointer_.Status(time);
}

EpStatus Endpointer::ProcessAudio(const qint16* audio_data, int num_samples,
                                  float* rms_out) {
  ...
  while (sample_index + frame_size_ <= num_samples) {
    ...
    audio_frame_time_us_ += (frame_size_ * kUsecPerSec) /
                         sample_rate_;
    ...
    if (ep_time > speech_input_minimum_length_us_) {
      ...
      if (waiting_for_speech_complete_timeout_) {
        ...
        qint64 requested_silence_length;
