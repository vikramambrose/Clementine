...
bool GstEnginePipeline::Init() {
  ...
  // last bands as corner cases.
  // That was causing the "inverted slider" bug. As a workaround, we create two
  // dummy bands at both ends of the spectrum.
  // As a consequence, the actual first and last adjustable bands are
  // implemented using
  // band-pass filters.
  ...
  for (int i = 0; i < kEqBandCount; ++i) {
    ...
    qDebug() << "Adding band: " << index_in_eq << " " << lower_frequency << " ["
             << center_frequency << "] " << upper_frequency << " - "
             << bandwidth << " " << ISO_base10_coefficient;
